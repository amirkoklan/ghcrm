...
<div id="maincontent"{if $edit_menu_collapsed} style="margin-left:20px"{/if}>
    <div id="maincontent-design" class="float-break">
                ...
                <div class="box-bc">
                    <div class="box-ml">
                        <div class="button-left">
                            <input class="defaultbutton" type="submit" name="PublishButton" value="{if $object.published}{'Save and Send Later'|i18n( 'design/admin/content/edit' )}{else}{'Send for publishing'|i18n( 'design/admin/content/edit' )}{/if}" title="{'Publish the contents of the draft that is being edited. The draft will become the published version of the object. The email can be sent at a later time.'|i18n( 'design/admin/content/edit' )}" />
                            <input id="Publish" class="defaultbutton" type="submit" name="PublishButton" send="Send" value="{if $object.published}{'Send Email Now'|i18n( 'design/admin/content/edit' )}{else}{'Send for publishing'|i18n( 'design/admin/content/edit' )}{/if}" title="{'Send this email now. This activity has been completed.'|i18n( 'design/admin/content/edit' )}" />
                        ...
                        </div><!-- ends .button-left -->
                        <div class="button-right"></div><!-- ends .button-right -->
                        <div class="float-break"></div><!-- ends .float-break -->
                    {* DESIGN: Control bar END *}
                    </div><!-- ends .box-ml -->
                </div><!-- ends .box-bc -->
            </div><!-- ends #controlbar-top -->
        ...
        '%to_lang', concat( $translation_list[$language_index].locale.intl_language_name, '&nbsp;<img src="', $translation_list[$language_index].language_code|flag_icon, '" width="18" height="12" style="vertical-align: middle;" alt="', $translation_list[$language_index].language_code, '" />' ) ) )}
                    ...
                    </div><!-- ends .context-attributes -->

                    {if $is_translating_content}
                        </div><!-- ends .content-translation -->
                    {/if}

                {* DESIGN: Content END *}
                </div><!-- ends .box-content -->
                ...
                <div class="controlbar">
                    ...
                    <div class="block">
                        ...
                        {/if}
                        <input class="defaultbutton" type="submit" name="PublishButton" value="{if $object.published}{'Send Mail (if Verify - Send Email is checked)'|i18n( 'design/admin/content/edit' )}{else}{'Send for publishing'|i18n( 'design/admin/content/edit' )}{/if}" title="{'Publish the contents of the draft that is being edited. The draft will become the published version of the object.'|i18n( 'design/admin/content/edit' )}" />
                        <input class="button" type="submit" name="StoreButton" value="{'Store draft'|i18n( 'design/admin/content/edit' )}" title="{'Store the contents of the draft that is being edited and continue editing. Use this button to periodically save your work while editing.'|i18n( 'design/admin/content/edit' )}" />
                        <input class="button" type="submit" name="StoreExitButton" value="{'Store draft and exit'|i18n( 'design/admin/content/edit' )}" title="{'Store the draft that is being edited and exit from edit mode. Use when you need to exit your work and return later to continue.'|i18n( 'design/admin/content/edit' )}" />
                        <input class="button" type="submit" name="DiscardButton" value="{'Discard draft'|i18n( 'design/admin/content/edit' )}" onclick="return confirmDiscard( '{'Are you sure you want to discard the draft?'|i18n( 'design/admin/content/edit' )|wash(javascript)}' );" title="{'Discard the draft that is being edited. This will also remove the translations that belong to the draft (if any).'|i18n( 'design/admin/content/edit' ) }" />
            ...
            {/if}
                </div><!-- ends .content-edit -->
            </div><!-- ends .fix -->

            <div class="break"></div>
        </div><!-- ends .maincontent-design -->
...
$(document).ready(function(){
        $('.ezcca-crm_email_send_email').click( function(){
        if ($('.ezcca-crm_email_send_email').is(':checked'))
        {
