{* Template: crm_opportunities_list.tpl
   Purpose: Get opportunities of this organization
   Granite Horizon CRM *}

{def $count=fetch( 'content', 'list_count',
    hash( 'parent_node_id', $node.node_id,
            'class_filter_type', 'include',
            'class_filter_array', array( 'crm_opportunity' ) )
)}
<div class="content_right">
    <div class="crm-header">
        <h1>Opportunities</h1>
            ...
            {else}
                <div class="no_opportunities">(No Opportunities)</div><!-- ends no_opportunities -->
            {/if}
    </div><!-- ends crm-header -->
    ...
    <form class="form_button_dropdown" method="post" action={"/content/action"|ezurl()}>
        ...
        <input src={"add_new.png"|ezimage()} type="image" title="Add New Opportunity" class="add_new_button_dropdown" value="Submit" alt="Submit" />
    </form>
...
{def $counter=0}
    ...
    <ul>
        {foreach $node.children as $child}
            {if eq($child.object.content_class.identifier,'crm_opportunity')}
                {set $counter=inc( $counter )}
                <li>
                    <span class="opportunities_list">{node_view_gui content_node=$child view='line'}</span>
                    {if $child.can_edit}
